...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_Delay(3000); // 3 second window to get Python script running
  ...
  HAL_TIM_PWM_Start(&htim8, TIM_CHANNEL_3); // SH
  ...
  while (1)
  {
	  if (tim2_ready && !frame_done && !frame_sent)
	      ...
	      if (frame_done && !frame_sent)
	      {
	          ...
	          HAL_UART_Transmit(&huart1, (uint8_t*)CCDPixelBuffer, CCDBuffer * 2, HAL_MAX_DELAY);
	          frame_sent = 1;
	      }
